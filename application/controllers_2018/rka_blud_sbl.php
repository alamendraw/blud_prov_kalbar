...
class Rka_blud extends CI_Controller {
    ...
    function  dotrek($rek){
        		...
        		break;
				case 8:
					$rek = $this->left($rek,1).'.'.substr($rek,1,1).'.'.substr($rek,2,1).'.'.substr($rek,3,2).'.'.substr($rek,5,2).'.'.substr($rek,7,1);								
        		break;
    			default:
				$rek = "";	
				}
	...
	function select_rka($kegiatan='',$unit='') {
        ...
        $sql = "select a.kd_rek5,b.nm_rek5,a.nilai,a.nilai_ubah,a.sumber,a.sumber2,a.sumber3,a.sumber4 from trdrka_blud a inner join ms_rek5 b on a.kd_rek5=b.kd_rek5  where a.kd_kegiatan='$kegiatan'  order by a.kd_rek5";                   
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
	...
	function preview_rba(){
	   ...
	   $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat as pangkat FROM ms_ttd WHERE left(kd_skpd,7)= left('$id',7) AND kode='PA' AND(REPLACE(nip, ' ', '')='$ttd1')  ";
        ...
        $sqlorg="SELECT f.kd_urusan,f.nm_urusan,a.kd_skpd,e.nm_skpd,a.kd_program,a.nm_program,a.kd_kegiatan,c.nm_kegiatan,SUM(d.nilai) AS nilai,a.tu_capai,a.tu_mas,a.tu_kel,a.tu_has,
                ...
                INNER JOIN ms_urusan_blud f ON a.kd_urusan=f.kd_urusan where a.kd_kegiatan='$giat'
        ...
        $cRet .="<table style=\"border-collapse:collapse;font-size:12px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"0\" cellpadding=\"4\">
                    <tr> 
                         ...
                         <td width=\"10%\" rowspan=\"3\" align=\"center\"><strong>FORMULIR <BR> RBA BLUD RSUD  </strong></td>
                    </tr>
        ...
        $cRet .= "<table style=\"border-collapse:collapse;font-size:12px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"2\" cellpadding=\"2\">
                 ...
                 $sql1="SELECT * FROM(SELECT 0 header,0 no_po, LEFT(a.kd_rek5,1)AS rek1,LEFT(a.kd_rek5,1)AS rek,b.nm_rek1 AS nama ,0 AS volume,' 'AS satuan,
						...
						AND a.kd_skpd='$id'  GROUP BY a.kd_rek5,b.nm_rek5
						 UNION ALL 
						 SELECT * FROM (SELECT  b.header,b.no_po,RIGHT(a.no_trdrka,7) AS rek1,RTRIM(a.kd_rek5) AS rek,b.uraian AS nama,0 AS volume,' ' AS satuan,
						...
						GROUP BY  RIGHT(a.no_trdrka,7),b.header,a.kd_rek5,b.no_po,b.uraian)z WHERE header='1'
						UNION ALL
						SELECT a. header,a.no_po,RIGHT(a.no_trdrka,7) AS rek1,' 'AS rek,a.uraian AS nama,a.volume1 AS volume,a.satuan1 AS satuan,
						a.harga1 AS harga,a.total AS nilai,'6' AS id FROM trdpo_blud a  WHERE LEFT(a.no_trdrka,10)='$id' AND SUBSTRING(no_trdrka,12,21)='$giat' AND (header='0' or header is null)
	   ...
	   $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat as pangkat FROM ms_ttd WHERE left(kd_skpd,7)= left('$id',7) AND kode='PA' AND(REPLACE(nip, ' ', '')='$ttd1')  ";
        ...
        $cRet .= "<table style=\"border-collapse:collapse;font-size:12px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"2\" cellpadding=\"2\">
                 ...
                 $sql1="SELECT * FROM(SELECT 0 header,0 no_po, LEFT(a.kd_rek5,1)AS rek1,LEFT(a.kd_rek5,1)AS rek,b.nm_rek1 AS nama ,
						...
						AND a.kd_skpd='$id'  GROUP BY a.kd_rek5,b.nm_rek5
						 UNION ALL 
						 SELECT * FROM (SELECT  b.header,b.no_po,RIGHT(a.no_trdrka,7) AS rek1,RTRIM(a.kd_rek5) AS rek,b.uraian AS nama,
						...
						'6' AS id FROM trdpo_blud a  WHERE LEFT(a.no_trdrka,10)='$id' AND SUBSTRING(no_trdrka,12,21)='$giat' AND (header='0' or header is null)
						) a ORDER BY a.rek1,a.id,a.no_po";
