...
class M_Transout extends CI_Model {
	...
	function load_rek($giat,$skpd,$nomor,$sp2d){
	...
	function simpan_transout($tabel,$nomor,$tgl,$nokas,$tglkas,$skpd,$nmskpd,$beban,$ket,$total,$csql,$usernm,$xpay,$update,$jns_tagih,$no_tagih){
		$msg = array ();
		if ($tabel == 'trhtransout_blud') {
            ...
            if ($asgbl) {
                        $sqlbl = "insert into trhtransout_blud(no_kas,tgl_kas,no_bukti,tgl_bukti,ket,username,tgl_update,kd_skpd,nm_skpd,total,pay,no_tagih,sts_tagih,jns_spp) 
                        values('$nokas','$tglkas','$nomor','$tgl','$ket','$usernm','$update','$skpd','$nmskpd','$total','$xpay','$no_tagih','$jns_tagih','$beban')";
                        $asgbl = $this->db->query ( $sqlbl );
                       if (! ($sqlbl)) {
                            $msg = array (
                              'pesan' => '0' 
        ...
        elseif ($tabel == 'trdtransout_blud') {
            ...
            // Simpan Detail //
            $sqlbl = "delete from trdtransout_blud where no_bukti='$nomor'";
            $asgbl = $this->db->query ( $sqlbl );
            if (! ($sqlbl)) {
                $msg = array (
                        'pesan' => '0' 
	...
	function update_transout($tabel,$nomor,$tgl,$nokas,$tglkas,$skpd,$nmskpd,$beban,$ket,$total,$csql,$usernm,$xpay,$update,$nomor_simpan,$jns_tagih,$no_tagih){
		$msg = array ();
        // Simpan Header //
        if ($tabel == 'trhtransout_blud') {
        ...
        elseif ($tabel == 'trdtransout_blud') {
            ...
            // Simpan Detail //
            $sqlbl = "delete from trdtransout_blud where no_bukti='$nomor_simpan'";
            $asgbl = $this->db->query ( $sqlbl );
            if (! ($sqlbl)) {
                $msg = array (
                        'pesan' => '0' 
	...
	function load_giat_trans($sp2d,$cskpd){
        $lccr = $this->input->post ( 'q' );
        $sql = "SELECT a.kd_kegiatan,a.nm_kegiatan FROM trhtransout_blud a 
				INNER JOIN trdtransout_blud b ON a.kd_skpd=b.kd_skpd AND a.no_bukti=b.no_bukti
				WHERE a.kd_skpd='$cskpd' AND a.no_sp2d='$sp2d'
				GROUP BY a.kd_kegiatan,a.nm_kegiatan 
				ORDER BY a.kd_kegiatan,a.nm_kegiatan";
        $query1 = $this->db->query ( $sql );
        $result = array ();
	...
	function load_pengembalian($kd_skpd,$kriteria){
        ...
        if ($kriteria <> '') {
            ...
            $where1 = " WHERE (upper(no_sts) like upper('%$kriteria%') or tgl_sts like '%$kriteria%' or upper(keterangan) like upper('%$kriteria%')) ";
        }
        
        $sql = "SELECT count(*) as tot from trhkasin_blud $where1";
       $query1 = $this->db->query($sql);
        $total = $query1->row();
        
        $sql = "SELECT TOP $rows * FROM trhkasin_blud WHERE no_sts NOT IN (SELECT TOP $offset no_sts FROM trhkasin_blud $where1 ORDER BY no_sts) $where order by no_sts";
        $query1 = $this->db->query ( $sql );
        $result = array();
        $ii = 0;
        foreach ( $query1->result_array () as $resulte ) {
	...
	function load_dpengembalian($nomor,$skpd){
		$sql = "SELECT * FROM trdkasin_blud WHERE no_sts ='$nomor' AND kd_skpd='$skpd'";
        $query1 = $this->db->query ( $sql );
        $result = array ();
        $ii = 0;
        foreach ( $query1->result_array () as $resulte ) {
	...
	function hapus_pengembalian($nomor,$skpd){
        ...
        $asg = $this->db->query ( $sql );
        if ($asg) {
            $sql = "delete from trhkasin_blud  where no_sts='$nomor' AND kd_skpd='$skpd'";
            $asg = $this->db->query ( $sql );
            $pesan = $this->db->last_query();
            $this->tukd_model->set_log_activity($pesan);
            if (! ($asg)) {
