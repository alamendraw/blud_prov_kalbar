...
class Rka_model extends CI_Model {
	...
	{
		$this->db->select('trskpd_blud.kd_urusan,trskpd_blud.kd_skpd,trskpd_blud.kd_kegiatan as giat,m_giat_blud.nm_kegiatan');
		$this->db->from('trskpd_blud');
        $this->db->join('m_giat_blud','m_giat_blud.kd_kegiatan=trskpd_blud.kd_kegiatan1');
       	$this->db->where('trskpd_blud.kd_skpd', $id);
        $this->db->where('trskpd_blud.jns_kegiatan','52');
        $this->db->order_by('trskpd_blud.kd_skpd');
		$this->db->order_by('trskpd_blud.kd_kegiatan', 'asc');
		$this->db->limit($limit,$offset);
		return $this->db->get();
	}
	
		function getAll_blud($limit, $offset,$id)
	{
		$this->db->select('trskpd_blud.kd_urusan,trskpd_blud.kd_skpd,trskpd_blud.kd_kegiatan as giat,m_giat_blud.nm_kegiatan');
		$this->db->from('trskpd_blud');
        $this->db->join('m_giat_blud','m_giat_blud.kd_kegiatan=trskpd_blud.kd_kegiatan1');
       	$this->db->where('trskpd_blud.kd_skpd', $id);
        $this->db->where('trskpd_blud.jns_kegiatan','52');
		$this->db->where('trskpd_blud.kd_kegiatan','1.02.1.02.02.00.33.01');
        $this->db->order_by('trskpd_blud.kd_skpd');
		$this->db->order_by('trskpd_blud.kd_kegiatan', 'asc');
		$this->db->limit($limit,$offset);
	 ...
	 function getAllSub($limit, $offset,$id)
    {
        $this->db->select('trskpd_blud.kd_urusan,trskpd_blud.kd_skpd,trskpd_blud.kd_kegiatan as giat,m_giat_blud.nm_kegiatan');
        $this->db->from('trskpd_blud');
        $this->db->join('m_giat_blud','m_giat_blud.kd_kegiatan=trskpd_blud.kd_kegiatan1');
        ...
        $this->db->order_by('trskpd_blud.kd_skpd');
        $this->db->order_by('trskpd_blud.kd_kegiatan', 'asc');
        $this->db->group_by('trskpd_blud.kd_kegiatan');
        $this->db->limit($limit,$offset);
        return $this->db->get();
    }
	...
	function combo_ttd($kode='') 
    {
        ...
        $csql    = "SELECT * from ms_ttd_blud where kd_skpd = '$kd_skpd' order by nama ";
    ...
    function cari($limit, $offset,$data)
	{
        ...
        $this->db->or_like('trskpd_blud.kd_kegiatan', $data);      
		$this->db->order_by('trskpd_blud.kd_kegiatan', 'asc');
		return $this->db->get();
	}
  ...
  function get_nama($kode,$hasil,$tabel,$field)
    ...
    function combo_urus($skpd='',$tahun='') 
    {
        ...
        $csql    = "SELECT * from ms_urusan_blud order by kd_urusan ";
        $query   = $this->db->query($csql);
    ...
    function combo_skpd($skpd='',$tahun='') 
    {
        ...
        $csql    = "SELECT * from ms_skpd_blud order by kd_skpd ";
        $query   = $this->db->query($csql);
    ...
    function combo_giat($giat='',$skpd=''){
        ...
        $csql    = "SELECT a.kd_kegiatan1,a.kd_kegiatan,b.nm_kegiatan FROM trskpd_blud a LEFT JOIN m_giat_blud b ON a.kd_kegiatan1=b.kd_kegiatan where a.kd_skpd like '%$skpd%' ORDER BY b.kd_kegiatan";
        $query   = $this->db->query($csql);
