...
class C_Transout extends CI_Controller {
	...
	function jumlah_ang_trans_lain(){  
		...
		$data	  = $this->M_Transout->jumlah_ang_trans_lain($ckdkegi,$ckdrek,$crekblud,$ckdskpd,$jns,$sp2d);				
		echo json_encode($data);
    }
	
	function load_no_ambil_kas() {
        $giat        = $this->input->post ( 'giat' );
        $kode        = $this->input->post ( 'kd' );
        $jns        = $this->input->post ( 'jns' );
		$data		= $this->M_Transout->load_no_ambil_kas($giat,$kode,$jns);			
		echo json_encode($data);
    }
	
	function load_no_ambil_silpa() {
        $giat        = $this->input->post ( 'giat' );
        $kode        = $this->input->post ( 'kd' );
        $jns        = $this->input->post ( 'jns' );
		$data		= $this->M_Transout->load_no_ambil_silpa($giat,$kode,$jns);			
		echo json_encode($data);
    }
	
	function load_rek_lain() {
        $giat        = $this->input->post ( 'giat' );
		...
		$data		= $this->M_Transout->load_rek_lain($giat,$jns,$skpd,$nomor,$sp2d);			
		echo json_encode($data);
    }
	
	function load_transout_lain() {
        $kd_skpd = $this->session->userdata ( 'kdskpd' );
		$kriteria = '';
        $kriteria = $this->input->post ( 'cari' );
		$data	= $this->M_Transout->load_transout_lain($kd_skpd,$kriteria);				
		echo json_encode($data);
    }
	
	function load_transout_silpa() {
        $kd_skpd = $this->session->userdata ( 'kdskpd' );
		$kriteria = '';
        $kriteria = $this->input->post ( 'cari' );
		$data	= $this->M_Transout->load_transout_silpa($kd_skpd,$kriteria);				
		echo json_encode($data);
    }
	
	function transout_blud_lain() {
        $data ['page_title'] = 'INPUT PEMBAYARAN TRANSAKSI LAIN-LAIN';
        $data['kdskpd']   = $this->session->userdata ('kdskpd' );
        $data['tahun']  = $this->session->userdata ( 'pcThang' );
        $this->template->set ( 'title', 'INPUT PEMBAYARAN TRANSAKSI LAIN-LAIN' );
        $this->template->load ( 'template', 'tukd/transaksi/transout_blud_lain', $data );
        $this->tukd_model->set_log_activity(); 
    }
	
		function transout_blud_silpa() {
        $data ['page_title'] = 'INPUT PEMBAYARAN TRANSAKSI  SILPA';
        $data['kdskpd']   = $this->session->userdata ('kdskpd' );
        $data['tahun']  = $this->session->userdata ( 'pcThang' );
        $this->template->set ( 'title', 'INPUT PEMBAYARAN TRANSAKSI DENGAN MENGGUNAKAN SILPA' );
        $this->template->load ( 'template', 'tukd/transaksi/transout_blud_silpa', $data );
        $this->tukd_model->set_log_activity(); 
    }
	...
	function transout_blud_tlalu() {
        $data ['page_title'] = 'INPUT PEMBAYARAN TRANSAKSI TAHUN LALU';
        $data['kdskpd']   = $this->session->userdata ('kdskpd' );
        $data['tahun']  = $this->session->userdata ( 'pcThang' );
        $this->template->set ( 'title', 'INPUT PEMBAYARAN TRANSAKSI TAHUN LALU' );
        $this->template->load ( 'template', 'tukd/transaksi/transout_blud_tlalu', $data );
        $this->tukd_model->set_log_activity(); 
    }
	...
	function load_transout_tlalu() {
        $kd_skpd = $this->session->userdata ( 'kdskpd' );
		$kriteria = '';
        $kriteria = $this->input->post ( 'cari' );
		$data	= $this->M_Transout->load_transout_tlalu($kd_skpd,$kriteria);				
		echo json_encode($data);
    } 
	...
	function load_rek() {
        ...
        $sp2d        = $this->input->post ( 'sp2d' );
        $lccr        = $this->input->post ( 'q' );
		$data		= $this->M_Transout->load_rek($giat,$jns,$skpd,$nomor,$sp2d);			
		echo json_encode($data);
    }
	...
	function simpan_transout() {
        ...
        $data		= $this->M_Transout->simpan_transout($tabel,$nomor,$tgl,$nokas,$tglkas,$skpd,$nmskpd,$beban,$ket,$total,$csql,$usernm,$xpay,$update,$jns_tagih,$no_tagih,$jenis);			
		echo json_encode($data);
    }
	...
	function update_transout() {
		...
		$jenis      = $this->input->post ( 'jenis' );
        $update		= date ( 'Y-m-d H:i:s' );
		$data		= $this->M_Transout->update_transout($tabel,$nomor,$tgl,$nokas,$tglkas,$skpd,$nmskpd,$beban,$ket,$total,$csql,$usernm,$xpay,$update,$nomor_simpan,$jns_tagih,$no_tagih,$jenis,$jenis);			
		echo json_encode($data);
    }
