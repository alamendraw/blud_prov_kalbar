...
class Tukd_cms extends CI_Controller {
    ...
    function config_npwp(){
        ...
        $sql = "SELECT top 1 npwp,rekening FROM ms_skpd_blud a WHERE a.kd_skpd = '$skpd'"; 
	...
	{		
        ...
        $sql = "SELECT top 1 rekening FROM ms_skpd_blud where $init_skpd order by kd_skpd";
        $query1 = $this->db->query($sql);  
        $result = array();
        foreach($query1->result_array() as $resulte)
        { 
	...
	{		
		$lccr =  $this->session->userdata('kdskpd');
        $sql = "SELECT top 1 rekening_pend FROM ms_skpd_blud where left(kd_skpd,7)=left('$lccr',7) order by kd_skpd";
        $query1 = $this->db->query($sql);  
        $result = array();
        foreach($query1->result_array() as $resulte)
        { 
    ...
    function cari_bank()
	{				
        $sql = "SELECT kode,nama FROM ms_bank_blud";
        $query1 = $this->db->query($sql);  
        $result = array();
        foreach($query1->result_array() as $resulte)
        { 
    ...
    function load_rek_bank() {
      $kd_skpd  = $this->session->userdata('kdskpd');
        ...
        $sql = "SELECT a.*,b.nama from ms_rekening_bank a left join ms_bank_blud b on b.kode = a.bank where a.kd_skpd = '$kd_skpd' $where order by nm_rekening,rekening ";//limit $offset,$rows";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function config_skpd(){
        ...
        $sql = "SELECT a.kd_skpd,a.nm_skpd FROM  ms_skpd_blud a WHERE a.kd_skpd = '$skpd'";
        $query1 = $this->db->query($sql);  
	...
	{				
        ...
        $cek_skpd = $this->db->query("select count(*) as hasil from ms_skpd_blud where kd_skpd='$skpd'")->row();
        ...
        $sql = "SELECT a.rekening,a.nm_rekening,a.bank,(select nama from ms_bank_blud where kode=a.bank) as nmbank,
	...
	{				
	    $skpd = $this->session->userdata('kdskpd');
        $sql = "SELECT a.rekening,a.nm_rekening,a.bank,(select nama from ms_bank_blud where kode=a.bank) as nmbank,a.kd_skpd,a.jenis FROM ms_rekening_bank a where a.jenis='$jenis'";
        $query1 = $this->db->query($sql);  
        $result = array();
        foreach($query1->result_array() as $resulte)
        { 
    ...
    function transout()
    {
        ...
        $kd_skpd  = $this->session->userdata('kdskpd');
        $cek_skpd = $this->db->query("select count(*) as hasil from ms_skpd_blud where kd_skpd='$kd_skpd'")->row();
    ...
    function load_transout_tunai(){
        ...
        $sql = "SELECT count(*) as total from trhtransout_blud a where a.panjar = '0' AND a.pay='TUNAI' AND $init_skpd $where " ;
    ...
    where v.jenis=a.jns_spp and z.no_bukti = a.no_bukti and z.kd_bp_skpd = a.kd_skpd) ketlpj,
        ...
        WHERE  a.panjar = '0' AND a.pay='TUNAI' AND $init_skpd $where and a.no_bukti not in (SELECT top $offset a.no_bukti FROM trhtransout_blud a  
		...
		/*$sql = "SELECT TOP 70 PERCENT a.*,b.no_bukti AS nokas_pot,b.tgl_bukti AS tgl_pot,b.ket AS kete FROM trhtransout_blud a LEFT JOIN trhtrmpot b ON  a.no_kas_pot=b.no_bukti 
    ...
    function load_transout(){
        ...
        WHERE  a.panjar = '0' AND $init_skpd $where order by CAST (a.no_bukti as NUMERIC))  order by tgl_voucher,CAST (a.no_bukti as NUMERIC),kd_skpd ";
    ...
    function pot() {
        ...
        $sql = "SELECT * FROM trspmpot where no_spm='$spm' AND kd_skpd='$kd_skpd' order by kd_rek5 ";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function load_trskpd_sub() {        
        ...
        $sql ="select a.kd_kegiatan,a.nm_kegiatan,sum(a.nilai) as total from trdrka_blud a 				
	   where $par $cgiat AND left(a.kd_rek5,1)='5' AND (UPPER(a.kd_kegiatan) LIKE UPPER('%%') OR UPPER(a.nm_kegiatan) LIKE UPPER('%%'))
    ...
    function load_trskpd_sub_tunai() {        
        ...
        $sskpd = substr($cskpd,0,7);
        
        $bid = $this->session->userdata('kdskpd');
        $cek = explode(".",$bid);
        if($cek=="00"){
            $par = "left(b.kd_skpd,7)='$sskpd' and b.kd_rek5 in ('5220312','5220104','5220501','5222501','5220307','5220505')";
            $par2 = "left(b.kd_skpd,7)='$sskpd'";
        }else{
            $par = "b.kd_skpd='$bid' and b.kd_rek5 in ('5220312','5220104','5220501','5222501','5220307','5220505')";
            $par2 = "b.kd_skpd='$bid'";
        }
	...
	inner join m_giat a on b.kd_kegiatan = a.kd_kegiatan
	inner join m_prog c on c.kd_program = a.kd_program
	where $par $jns_beban $cgiat AND (UPPER(a.kd_kegiatan) LIKE UPPER('%%') OR UPPER(a.nm_kegiatan) LIKE UPPER('%%'))
	...
	inner join m_giat a on b.kd_kegiatan = a.kd_kegiatan
	inner join m_prog c on c.kd_program = a.kd_program
	where $par2 $jns_beban AND a.kd_kegiatan='1.01.1.01.01.00.22.002' AND b.kd_rek5='5221104' AND (UPPER(a.kd_kegiatan) LIKE UPPER('%%') OR UPPER(a.nm_kegiatan) LIKE UPPER('%%'))
	group by  a.kd_kegiatan,a.nm_kegiatan,c.kd_program,c.nm_program
    ...
    function load_sp2d_transout(){
       ...
       if ($beban=='1'){
        $sisa = "c.nilai + (SELECT SUM(ISNULL (v.nilai,0)) FROM trhspp z INNER JOIN trhspm s ON z.no_spp=s.no_spp AND z.kd_skpd=s.kd_skpd
                INNER JOIN trhsp2d_blud v ON s.no_spm=v.no_spm AND s.kd_skpd=v.kd_skpd WHERE z.jns_spp IN ('1','2') AND z.kd_skpd=c.kd_skpd )
                -(SELECT SUM(ISNULL (nilai,0)) FROM trdtransout_blud WHERE no_sp2d = c.no_sp2d and no_bukti <> '$bukti') AS sisa";
       }else{
        $sisa = "c.nilai -(SELECT SUM(ISNULL (nilai,0))FROM trdtransout_blud WHERE no_sp2d = c.no_sp2d and no_bukti <> '$bukti') AS sisa";   
       }
       ...
       if ($giat !='' && $beban != '1'){
        ...
        $kriteria = $this->input->post('q');
            $sql = "SELECT DISTINCT a.no_sp2d,a.tgl_sp2d,sum(a.nilai) as nilai,
                    ...
                    WHERE LEFT(a.kd_skpd,7) = LEFT('$kode',7) AND a.status = 1 $where 
    ...
    function perusahaan() {                 
        ...
        $sql = "
                SELECT z.* FROM (
                SELECT rekanan as nmrekan, pimpinan, npwp, alamat FROM trhsp2d_blud WHERE LEN(rekanan)>1 AND kd_skpd = '$kd_skpd'   
					AND UPPER(rekanan) LIKE UPPER('%$lccr%')
					GROUP BY rekanan, pimpinan, npwp, alamat
				UNION ALL
    ...
    function load_rek(){
        ...
        if ($jenis=='1' || $jenis=='2'){
            $sql = "SELECT distinct a.kd_rek5,a.nm_rek5,c.kd_rek5 kd_rek_blud, c.nm_rek5 nm_rek_blud FROM trdrka_blud a 
					...
					WHERE a.kd_skpd='$kode' AND a.kd_kegiatan='$giat'";
            }
        else {
            $sql = "SELECT b.kd_rek5,c.nm_rek5,b.kd_rek_blud,nm_rek_blud FROM trhsp2d_blud a 
					...
					WHERE a.kd_skpd='$kode' AND a.no_sp2d='$sp2d' AND b.kd_kegiatan='$giat'";
        }
    ...
    function load_rek_total() {                      
        ...
        if ($ckdrek !=''){        
            $notIn = " and a.kd_rek5 not in ($ckdrek) " ;
        }else{
            $notIn  = "";
        }
			
        if($jns=='1' || $jns=='2'){
        $sql="SELECT ISNULL(SUM(a.nilai),0) as nilai, ISNULL(SUM(a.nilai),0) as nilai_sempurna,ISNULL(SUM(a.nilai_ubah),0) as nilai_ubah,
			(
			...
			AND jns_spp='1'
			UNION ALL
            SELECT SUM(a.nilai) as nilai FROM trdtransout_cmsbank a INNER JOIN trhtransout_cmsbank b 
			ON a.no_voucher=b.no_voucher AND a.kd_skpd=b.kd_skpd
			WHERE a.kd_kegiatan='$ckdkegi' AND a.kd_rek5='$ckdrek'
			AND jns_spp='1'			
            UNION ALL
			...
			$sql="SELECT ISNULL(SUM(b.nilai),0) as nilai,ISNULL(SUM(b.nilai),0) as nilai_sempurna,ISNULL(SUM(b.nilai),0) as nilai_ubah, 
            ...
            if($giat=='1.01.1.01.01.00.22.002'){
                ...
                $sql = "SELECT a.kd_rek5,a.nm_rek5,
                    (SELECT SUM(nilai) FROM 
						...
						0 AS sp2d,nilai AS anggaran,nilai_sempurna as nilai_sempurna, nilai_ubah AS nilai_ubah
						FROM trdrka a WHERE a.kd_kegiatan= '$giat' AND a.kd_rek5 in ('5220312','5220104','5220501','5222501','5220307','5220505') AND a.kd_skpd = '$kode' $notIn  ";
    ...
    function load_reksumber_dana() {                      
        ...
        $lccr   = $this->input->post('q');
            ...
            a.kd_kegiatan='$giat' and a.kd_rek5='$rek' and left(a.kd_skpd,7)=left('$kode',7)
            union ALL
    ...
    function load_sisa_bank(){
        ...
        if($cek_skpd1==1){
            ...
            $init_skpd = "left(kode,7)=left('$kd_skpd',7)";
        }
			...
			select a.tgl_sts as tgl,a.no_sts as bku, a.keterangan as ket, SUM(b.rupiah) as jumlah, '2' as jns, a.kd_skpd as kode 
					...
					where jns_trans NOT IN('4','2') and pot_khusus=2  AND left(a.kd_skpd,7)=left('$kd_skpd',7)
			...
			SELECT tgl_bukti AS tgl,no_bukti AS bku,ket AS ket,total-isnull(pot,0) AS jumlah,'2' AS jns,a.kd_skpd AS kode FROM trhtransout_blud a join trhsp2d b on a.no_sp2d=b.no_sp2d left join (select no_spm, kd_skpd, sum(nilai)pot from trspmpot group by no_spm,kd_skpd) c on b.no_spm=c.no_spm AND b.kd_skpd=c.kd_skpd WHERE pay='BANK' UNION ALL
            ...
            SELECT tgl_kas AS tgl,no_kas AS bku,keterangan AS ket,nilai AS jumlah,'2' AS jns,kd_skpd AS kode FROM tr_ambilsimpanan WHERE left(kd_skpd,7)=left('$kd_skpd',7) AND status_drop !='1' union all
            SELECT tgl_kas AS tgl,no_kas AS bku,keterangan AS ket,nilai AS jumlah,'2' AS jns,kd_skpd_sumber AS kode FROM tr_setorpelimpahan_bank WHERE left(kd_skpd,7)=left('$kd_skpd',7)
			) a
				where $init_skpd");
        //}
    ...
    function load_sisa_bank_cms(){
        ...
        if($cek_skpd1==1){
			...
			SELECT tgl_bukti AS tgl,no_bukti AS bku,ket AS ket,total-isnull(pot,0) AS jumlah,'2' AS jns,a.kd_skpd AS kode FROM trhtransout_blud a join trhsp2d b on a.no_sp2d=b.no_sp2d left join (select no_spm, kd_skpd, sum(nilai)pot from trspmpot group by no_spm,kd_skpd) c on b.no_spm=c.no_spm AND b.kd_skpd=c.kd_skpd WHERE pay='BANK' UNION ALL
			SELECT tgl_kas AS tgl,no_kas AS bku,keterangan AS ket,nilai AS jumlah,'2' AS jns,kd_skpd AS kode FROM tr_ambilsimpanan WHERE left(kd_skpd,7)=left('$kd_skpd',7) AND status_drop !='1' union all
            SELECT tgl_kas AS tgl,no_kas AS bku,keterangan AS ket,nilai AS jumlah,'2' AS jns,kd_skpd_sumber AS kode FROM tr_setorpelimpahan_bank_cms WHERE left(kd_skpd,7)=left('$kd_skpd',7)
			) a
				where $init_skpd");
        //}
    ...
    function load_total_anggaran(){
	   $kode    = $this->input->post('kode');
       $giat    = $this->input->post('giat');
       
       $init = $this->db->query("select 
                case when statu=1 and status_ubah=1 then 'Perubahan' 
				when statu=1 and status_ubah=0 then 'Penyusunan'
				else 'Penyusunan' end as anggaran from trhrka_blud where kd_skpd ='$kode'")->row();
       $status = $init->anggaran;         
       if($status=='Perubahan'){
            $n="nilai_ubah";
       }else{
            $n="nilai";
            ...
            $sql = "SELECT
						ISNULL(SUM($n),0) AS total_spd
					FROM
						trdrka_blud
					WHERE
						kd_skpd = '$kode'
					AND kd_kegiatan = '$giat'
					";
    ...
    function load_total_trans(){
	   $ckdskpd      = $this->input->post('kode');
       $ckdkegi    = $this->input->post('giat');
       $no_bukti    = $this->input->post('no_simpan');
		...
		$sql="SELECT ISNULL(SUM(a.nilai),0) as nilai, ISNULL(SUM(a.nilai),0) as nilai_sempurna,ISNULL(SUM(a.nilai_ubah),0) as nilai_ubah,
            ...
            UNION ALL
			SELECT SUM(a.nilai) as nilai FROM trdspp_blud a INNER JOIN trhsp2d_blud b ON a.kd_skpd=b.kd_skpd AND a.no_spp=b.no_spp
			WHERE jns_spp IN ('3','4','5','6') AND a.kd_kegiatan='$ckdkegi'
			UNION ALL
			SELECT SUM(a.nilai) as nilai FROM trdtagih_blud a INNER JOIN trhtagih_blud b ON a.kd_skpd=b.kd_skpd AND a.no_bukti=b.no_bukti
			WHERE a.kd_kegiatan='$ckdkegi'
			AND b.no_bukti NOT IN (select ISNULL(no_tagih,'') FROM trhsp2d_blud WHERE kd_skpd='$ckdskpd')  
        ...
        {                               
            $result[] = array(
                        'id' => $ii,        
                        'total' => number_format($resulte['lalu'],2,'.',',') 
                        );
                        $ii++;
        }
    ...
    function load_sisa_tunai(){
        ...
        $query1 = $this->db->query("
				...
				select a.tgl_sts as tgl,a.no_sts as bku, a.keterangan as ket, SUM(b.rupiah) as jumlah, '2' as jns, a.kd_skpd as kode 
					from trhkasin_pkd a INNER JOIN trdkasin_pkd b ON a.no_sts=b.no_sts AND a.kd_skpd=b.kd_skpd 
					where jns_trans NOT IN('4','2') and pot_khusus =0  AND $init_skpd
				...
				SELECT	a.tgl_bukti AS tgl,	a.no_bukti AS bku, a.ket AS ket, SUM(z.nilai) - isnull(pot, 0) AS jumlah, '2' AS jns, a.kd_skpd AS kode
								...
								where no_sp2d in 
								(SELECT no_sp2d as no_bukti FROM trhtransout_blud where $init_skpd2 GROUP BY no_sp2d HAVING COUNT(no_sp2d)>1)
								 and  no_kas not in
								(SELECT min(z.no_kas) as no_bukti FROM trhtransout_blud z WHERE z.jns_spp in (4,5,6) and $init_skpd2 
    ...
    function load_sisa_pot_ls(){
		...
		and b.no_sp2d = '$sp2d' and LEFT(b.kd_skpd,7) = LEFT('$kd_skpd',7) ");  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
        { 
	...
	select no_bukti nomor, 'Pembayaran Transaksi' ket, kd_skpd from trhtransout_blud where  isnumeric(no_bukti)=1 AND (panjar !='3' OR panjar IS NULL) union ALL
    select no_bukti nomor, 'Koreksi Transaksi' ket, kd_skpd from trhtransout_blud where  isnumeric(no_bukti)=1 AND panjar ='3' union ALL
   ...
   function no_urut_cms(){
    ...
    select no_bukti nomor, 'Potongan Pajak Transaksi Non Tunai' ket, kd_skpd from trhtrmpot_cmsbank where kd_skpd = '$kd_skpd') z WHERE KD_SKPD = '$kd_skpd'");
	    $ii = 0;
        foreach($query1->result_array() as $resulte)
        { 
            $result = array(
    ...
    $init_skpd = "b.kd_skpd = '$kd_skpd'";
    $init_skpd2 = "z.kd_skpd = '$kd_skpd'";
    ...
    $cek_skpd = $this->db->query("select count(*) as hasil from ms_skpd_blud where kd_skpd='$kd_skpd'")->row();
        ...
        if($cek_skpd1==1){
            ...
            if(substr($kd_skpd,8,2)=='00'){
                $init_skpd = "left(kd_skpd,7) = left('$kd_skpd',7)";
            }else{
                $init_skpd = "KD_SKPD = '$kd_skpd'";
            }            
        }
	...
	$query1 = $this->db->query("select case when max(nomor+1) is null then 1 else max(nomor+1) end as nomor from (
	select no_validasi nomor, 'Urut Validasi cms' ket, kd_skpd as kd_skpd from trvalidasi_cmsbank where kd_bp = '$kd_skpd' 
    ) 
    z WHERE $init_skpd ");
	    $ii = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function hapus_transout_tunai(){
		...
		if ($asg){
            $sql = "delete from trhtransout_blud where no_bukti='$nomor' AND kd_skpd='$kd_skpd' AND pay='TUNAI'";
            $asg = $this->db->query($sql);
        ...
        if ($tabel == 'trhtransout_cmsbank') {
            ...
            // Simpan Detail //                                       
				...
				if (!($asg)){
                    ...
                    $sql = "insert into trdtransout_cmsbank(no_voucher,no_sp2d,kd_kegiatan,nm_kegiatan,kd_rek5,nm_rek5,nilai,kd_skpd,sumber)"; 
    ...
    function simpan_transout_tunai(){
        ...
        if ($tabel == 'trhtransout_blud') {
            $sql = "delete from trhtransout_blud where kd_skpd='$skpd' and no_bukti='$nomor' and pay='TUNAI'";
			$asg = $this->db->query($sql);
            ...
            // Simpan Detail //                                       
				...
				if (!($asg)){
                    ...
                    $sql = "insert into trdtransout_blud (no_bukti,no_sp2d,kd_kegiatan,nm_kegiatan,kd_rek5,nm_rek5,nilai,kd_skpd,sumber)"; 
                    $asg = $this->db->query($sql.$csql);
...
function simpan_bataluploadcms(){
	...
	if($tabel == 'trvalidasi_cmsbank') {
                    ...
                    if (!($asg)){
                       ...
                       $sql = "UPDATE
                        ...
                        where a.kd_bp='$skpd' and no_validasi='$nval') AS Table_B ON trhtransout_cmsbank.no_voucher = Table_B.no_voucher AND trhtransout_cmsbank.kd_skpd = Table_B.kd_skpd
                        where trhtransout_cmsbank.kd_skpd='$skpd'";
                        $asg = $this->db->query($sql);
                        ...
                        if (!($asg)){
                            ...
                            $sql = "INSERT INTO trhtransout_blud (no_kas, tgl_kas, no_bukti, tgl_bukti, no_sp2d, ket, username, tgl_update, kd_skpd, nm_skpd, total, no_tagih, sts_tagih, tgl_tagih, jns_spp, pay, no_kas_pot,jenis)
                                    SELECT b.no_bukti as no_kas, b.tgl_validasi as tgl_kas, a.no_bukti, a.tgl_bukti, a.no_sp2d, a.ket, b.kd_skpd as username, a.tgl_update, b.kd_skpd, a.nm_skpd, a.total, a.no_tagih, a.sts_tagih, a.tgl_tagih, a.jns_spp, a.pay, a.no_kas_pot,'10'
                            ...
                            $asg = $this->db->query($sql);
                                ...
                                echo json_encode($msg);                     
                                    ...
                                    $sql = "INSERT INTO trdtransout_blud (no_bukti, no_sp2d, kd_kegiatan, nm_kegiatan, kd_rek5, nm_rek5, nilai, kd_skpd, kd_rek_blud,nm_rek_blud)
                                            SELECT c.no_bukti, a.no_sp2d, b.kd_kegiatan, b.nm_kegiatan, b.kd_rek5, b.nm_rek5, b.nilai, b.kd_skpd, b.sumber,d.nm_rek5
											...
											LEFT JOIN ms_rek5_blud d on b.sumber = d.kd_rek5
                                    ...
                                    if (!($asg)){
                                        ...
                                        //Hpotongan
                                        $sql = "INSERT INTO trhtrmpot_blud (no_bukti, tgl_bukti, ket, username, tgl_update, kd_skpd, nm_skpd, no_sp2d, nilai, npwp, jns_spp, status, kd_kegiatan, nm_kegiatan, kd_rek5, nm_rek5, nmrekan, pimpinan, alamat)
                                        SELECT cast(c.no_bukti as int)+1 as no_bukti, c.tgl_validasi as tgl_bukti, d.ket, d.username, d.tgl_update, d.kd_skpd, d.nm_skpd, d.no_sp2d, d.nilai, d.npwp, d.jns_spp, d.status, d.kd_kegiatan, d.nm_kegiatan, d.kd_rek5, d.nm_rek5, d.nmrekan, d.pimpinan, d.alamat 
                                        FROM trhtrmpot_cmsbank d LEFT JOIN trhtransout_cmsbank a on d.no_voucher=a.no_voucher and a.kd_skpd=d.kd_skpd
                                        LEFT JOIN trvalidasi_cmsbank c on c.no_voucher=a.no_voucher and a.kd_skpd=c.kd_skpd
                                        WHERE c.no_validasi='$nval' and a.status_trmpot='1' and c.kd_bp='$skpd'";
                                            ...
                                            if (!($asg)){
                                                ...
                                                echo json_encode($msg);                     
                                                    ...
                                                    SELECT cast(c.no_bukti as int)+1 as no_bukti, b.kd_rek5, b.nm_rek5, b.nilai, b.kd_skpd, b.kd_rek_trans,e.sumber
                                                    ...
                                                    LEFT JOIN trvalidasi_cmsbank c on c.no_voucher=a.no_voucher and a.kd_skpd=c.kd_skpd
													LEFT JOIN trdtransout_cmsbank e on e.no_voucher=a.no_voucher and a.kd_skpd=e.kd_skpd
                                                    WHERE c.no_validasi='$nval' and a.status_trmpot='1' and c.kd_bp='$skpd'";
                                                    $asg = $this->db->query($sql);                                    
	...
	if($tabel == 'trvalidasi_cmsbank') {
                    ...
                    //hapus Htrans   
                    $sql ="delete from trhtransout_blud where no_bukti='$nbku' and kd_skpd='$skpd'";
                    $asg = $this->db->query($sql);   
                    ...
                    if (!($asg)){
                       ...
                       $sql ="delete from trdtransout_blud where no_bukti='$nbku' and kd_skpd='$skpd'";
                       $asg = $this->db->query($sql);   
                                    ...
                                    if (!($asg)){
                                        ...
                                        $sql = "select count(*) as jml from trhtransout_cmsbank where no_voucher='$nval' and kd_skpd='$skpd' and status_trmpot='1'";
                                            $asg = $this->db->query($sql)->row();                                    
                                                ...
                                                $sql = "delete trhtrmpot_blud where no_bukti='$nbku_i' and kd_skpd='$skpd'";
                                                $asg = $this->db->query($sql);                                    
                                                ...
                                                if (!($asg)){
                                                    ...
                                                    $sql = "delete trdtrmpot_blud where no_bukti='$nbku_i' and kd_skpd='$skpd'";
                                                    $asg = $this->db->query($sql);                                    
                                                    ...
                                                    if (!($asg)){
                                                        ...
                                                        $sql = "delete trdtransout_transfercms where no_bukti='$nbku' and kd_skpd='$skpd'";
                                                        $asg = $this->db->query($sql);                                    
    ...
    function simpan_transout_edit(){
   	...
   	function load_pot_in(){
		...
		$sql = "SELECT top $rows a.*,(select status_upload from trhtransout_cmsbank where no_voucher=a.no_voucher and kd_skpd='$kd_skpd') as status_uploadx from trhtrmpot_cmsbank a where a.kd_skpd='$kd_skpd' AND a.no_bukti not in (SELECT top $offset no_bukti FROM trhtrmpot_cmsbank where kd_skpd='$kd_skpd' 
    ...
    function hapus_trmpot(){
        ...
        $sql = "update trhtransout_cmsbank set status_trmpot = '0' where no_voucher='$nomorvo' and kd_skpd='$kd_skpd'";
        $asg = $this->db->query($sql);
    ...
    function out_lalu(){
         ...
         $sql = "SELECT b.no_bukti,b.no_sp2d,b.kd_kegiatan,b.nm_kegiatan,b.kd_subkegiatan,b.nm_subkegiatan,b.kd_rek5,b.nm_rek5,
                (SELECT SUM(c.nilai) FROM trdtransout_blud c LEFT JOIN trhtransout_blud d ON c.no_bukti=d.no_bukti WHERE c.kd_subkegiatan = b.kd_subkegiatan AND 
                ...
                (SELECT SUM(nilai) FROM trdrka WHERE kd_kegiatan = b.kd_kegiatan AND kd_skpd=a.kd_skpd AND kd_rek5=b.kd_rek5) AS anggaran FROM trhtransout_blud a INNER JOIN trdtransout_blud b ON a.no_bukti=b.no_bukti
...
b.rekening_awal,b.nm_rekening_tujuan,b.rekening_tujuan,b.bank_tujuan,b.ket_tujuan,c.bersih
    ...
    function csv_cmsbank($nomor=''){
        ...
        $obskpd = $this->tukd_model->get_nama($skpd,'obskpd','ms_skpd_blud','kd_skpd');
        
        $cRet ='';
        $data='';
        $jdul='OB';                 
        //and a.tgl_upload='$tgl'
        $sqlquery = $this->db->query("SELECT a.tgl_upload,a.kd_skpd,(SELECT obskpd from ms_skpd_blud where kd_skpd=b.kd_skpd) as nm_skpd,
        ...
        where $init_skpd and a.no_upload='$nomor' and d.bank='05'");
 ...
 function csv_cmsbank_lain($nomor=''){
        ...
        left join trdtransout_transfercms c on b.kd_skpd=c.kd_skpd and c.no_voucher=b.no_voucher and c.tgl_voucher=b.tgl_voucher
        left join ms_rekening_bank d on RTRIM(d.rekening)=RTRIM(c.rekening_tujuan) and d.kd_skpd=b.kd_bp
        left join ms_bank_blud e on e.kode=d.bank
        where $init_skpd and a.no_upload='$nomor' and d.bank<>'05'");
        ...
        {            
            ...
            //$data = $resulte['nm_skpd'].",".$resulte['rekening_awal'].",".$resulte['nm_rekening_tujuan'].",".$resulte['rekening_tujuan'].",".$resulte['nilai'].",".$resulte['ket_tujuan']."\n";    
            $data = $resulte['nm_skpd'].";".str_replace(" ","",rtrim($rkawal)).";".rtrim($resulte['nm_rekening_tujuan']).";".rtrim($resulte['bic']).";".str_replace(" ","",rtrim($rwtujuan)).";".$nilai.";".$idr.";".$resulte['ket_tujuan']."\n";             
...
a.bank_tujuan,a.ket_tujuan,a.status_trmpot,c.no_upload
    ...
    function cetak_listtransaksi(){
        ...
        $sqlsc="SELECT tgl_rka,provinsi,kab_kota,daerah,thn_ang FROM sclient_blud WHERE kd_skpd=left('$kd_skpd',7)+'.00'";
         ...
         $cRet .="<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"1\" cellpadding=\"1\">
            ...
            <tr>
                ...
                <td align=\"left\" colspan=\"14\" style=\"font-size:12px;border: solid 1px white;\">:&nbsp;".strtoupper($this->tukd_model->get_nama($kd_skpd,'nm_skpd','ms_skpd_blud','kd_skpd'))."</td>
            </tr>
            </table>";
           ...
           if($cek_skpd1==1){
                ...
                $cek_skpd = substr($kd_skpd,8,2);           
                if($cek_skpd==00){
                $init_skpd = "LEFT(a.kd_skpd,7)=LEFT('$kd_skpd',7)";
                $init_skpd2= "left(kode,7)=left('$kd_skpd',7)";
                }else{
                $init_skpd = "a.kd_skpd='$kd_skpd'";
                $init_skpd2= "left(kode,7)=left('$kd_skpd',7)";
                }              
           ...
           $sql = "select z.* from (
            ...
            from trhtransout_cmsbank a where year(a.tgl_voucher) = '$thn' and a.tgl_voucher='$tgl' and $init_skpd
            UNION
            select '2' urut,a.kd_skpd,a.tgl_voucher,a.no_voucher,b.kd_kegiatan kegiatan,b.kd_rek5 rekening, b.nm_kegiatan+', '+b.nm_rek5, 0 terima, b.nilai keluar, a.jns_spp, '' status_upload
            from trhtransout_cmsbank a 
            left join trdtransout_cmsbank b on b.no_voucher=a.no_voucher and b.kd_skpd=a.kd_skpd
            where year(a.tgl_voucher) = '$thn' and a.tgl_voucher='$tgl' and $init_skpd
            UNION
            ...
            UNION
            select '4' urut,a.kd_skpd,a.tgl_voucher,a.no_voucher,b.kd_kegiatan kegiatan,c.kd_rek5 rekening, 'Terima '+c.nm_rek5, c.nilai terima, 0 keluar, '' jns_spp, '' status_upload
            from trhtransout_cmsbank a 
           ...
           foreach ($hasil->result() as $row)
            ...
            SUM(case when jns=2 then jumlah else 0 end) AS keluar
            from (
            SELECT tgl_kas AS tgl,no_kas AS bku,keterangan as ket,nilai AS jumlah,'1' AS jns,kd_skpd AS kode FROM tr_setorsimpanan_blud union ALL
            SELECT tgl_bukti AS tgl,no_bukti AS bku,ket as ket,nilai AS jumlah,'1' AS jns,kd_skpd AS kode FROM trhinlain_blud WHERE pay='BANK' union ALL
            SELECT tgl_bukti AS tgl,no_bukti AS bku,ket AS ket,a.total-isnull(pot,0) AS jumlah,'2' AS jns,a.kd_skpd AS kode FROM trhtransout_blud a join trhsp2d_blud b on a.no_sp2d=b.no_sp2d left join (select no_spm, sum(nilai)pot from trspmpot_blud group by no_spm) c on b.no_spm=c.no_spm WHERE pay='BANK' UNION ALL
        ...
        $sqlsc="SELECT tgl_rka,provinsi,kab_kota,daerah,thn_ang FROM sclient_blud WHERE kd_skpd=left('$kd_skpd',7)+'.00'";
         ...
         $cRet .="<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"1\" cellpadding=\"1\">
           ...
           $sql = "select z.* from (
            ...
            where year(a.tgl_bukti) = '$thn' and a.tgl_bukti='$tgl' and LEFT(a.kd_skpd,7)=LEFT('$kd_skpd',7)             
           ...
           foreach ($hasil->result() as $row)
            ...
            SELECT tgl_bukti AS tgl,no_bukti AS bku,ket AS ket,total-isnull(pot,0) AS jumlah,'2' AS jns,a.kd_skpd AS kode FROM trhtransout_blud a join trhsp2d b on a.no_sp2d=b.no_sp2d left join (select no_spm, sum(nilai)pot from trspmpot group by no_spm) c on b.no_spm=c.no_spm WHERE pay='BANK' UNION ALL
            ...
            SELECT tgl_bukti AS tgl,no_bukti AS bku,ket as ket,nilai AS jumlah,'2' AS jns,kd_skpd AS kode FROM trhoutlain WHERE pay='BANK') a
            where tgl<='$tgl' and left(kode,7)=left('$kd_skpd',7)";
...
and a.kd_skpd=b.kd_skpd WHERE a.kd_skpd = '1.03.01.00' and b.tgl_sts<'2018-1-2'")->row();
    ...
    function load_liststsbelum_upload(){
        ...
        where left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='0' $and " ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
       	
        $query1 = $this->db->query("SELECT top $rows a.* FROM trhkasin_pkd_cms a 
        where left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='0' $and 
        and a.no_sts not in (SELECT top $offset a.no_sts FROM trhkasin_pkd_cms a  
        WHERE left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='0' $and order by cast(a.urut as int))
    ...
    function load_liststs_upload(){
        ...
        where left(a.kd_skpd,7)=left('$skpd',7) $and 
        and a.no_sts not in (SELECT top $offset a.no_sts FROM trhkasin_pkd_cms a  
        WHERE left(a.kd_skpd,7)=left('$skpd',7) $and order by cast(a.urut as int))
        order by cast(a.urut as int),a.kd_skpd");		
        $result = array();
        $ii     = 0;
        foreach($query1->result_array() as $resulte)
	...
	if($tabel == 'trdupload_sts_cmsbank') {
            // Simpan Detail //                       
				...
				if (!($asg)){
                    ...
                    $skpd = $this->session->userdata('kdskpd'); 
                    $sql = "insert into trhupload_sts_cmsbank(no_upload,tgl_upload,kd_skpd,total,no_upload_tgl) values ('$nomor','$update','$skpd','$total','$urut_tgl')";
                    $asg = $this->db->query($sql);
                    ...
                    $sql = "UPDATE
                        ...
                        trdupload_sts_cmsbank b on b.kd_bp=a.kd_skpd and a.no_upload=b.no_upload 
                        where b.kd_bp='$skpd' and a.no_upload='$nomor') AS Table_B ON trhkasin_pkd_cms.no_sts = Table_B.no_sts AND trhkasin_pkd_cms.kd_skpd = Table_B.kd_skpd
                        where left(trhkasin_pkd_cms.kd_skpd,7)=left('$skpd',7)
                        ";
                    $asg = $this->db->query($sql);
    ...
    function csv_cmsbank_sts($nomor=''){
        ...
        b.rekening_awal,b.nm_rekening_tujuan,b.rekening_tujuan,b.nilai,b.ket_tujuan,b.no_upload_tgl FROM trhupload_sts_cmsbank a left join trdupload_sts_cmsbank b on b.kd_bp=a.kd_skpd and a.no_upload=b.no_upload 
        where left(a.kd_skpd,7)=left('$skpd',7) and a.no_upload='$nomor'");
    ...
    function load_draf_upload_sts(){
        ...
        $query1 = $this->db->query("SELECT top $rows a.*,b.* FROM trhupload_sts_cmsbank a left join trdupload_sts_cmsbank b on b.kd_bp=a.kd_skpd and a.no_upload=b.no_upload 
        where left(a.kd_skpd,7)=left('$skpd',7) $and 
        and a.no_upload not in (SELECT top $offset a.no_upload FROM trhupload_sts_cmsbank a  
        WHERE left(a.kd_skpd,7)=left('$skpd',7) $and order by cast(a.no_upload as int))
        order by cast(a.no_upload as int),a.kd_skpd");		
        $result = array();
        $ii     = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function load_transout_bnk(){
        ...
        $sql = "SELECT count(*) as total from trhtransout_blud a where a.pay='BANK' and a.panjar = '0' AND $init_skpd $where 
                and a.no_bukti not in (select no_bukti from trhtransout_cmsbank a WHERE  a.panjar = '0' AND left(a.kd_skpd,7)=left('$kd_skpd',7) $where)" ;
        //$sql = "SELECT count(*) as total from trhtransout_blud a where a.kd_skpd='$kd_skpd' and a.jns_spp in ('1','2','3') $where " ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
       	$result["total"] = $total->total; 
        ...
        join trhlpj v on v.no_lpj = z.no_lpj
        where v.jenis=a.jns_spp and z.no_bukti = a.no_bukti and z.kd_bp_skpd = a.kd_skpd) ketlpj,
		0 ketspj,(select rekening from ms_skpd where kd_skpd='$kd_skpd') as rekening_awal FROM trhtransout_blud a  
        WHERE  a.panjar = '0' AND $init_skpd $where and a.pay='BANK' and a.no_bukti not in (SELECT top $offset a.no_bukti FROM trhtransout_blud a  
    ...
    function load_tgltransout_bnk(){
        ...
        join trhlpj v on v.no_lpj = z.no_lpj
        where v.jenis=a.jns_spp and z.no_bukti = a.no_bukti and z.kd_bp_skpd = a.kd_skpd) ketlpj,
		0 ketspj FROM trhtransout_blud a  
        WHERE  a.panjar = '0' AND $init_skpd $where and a.pay='BANK' and a.no_bukti not in (SELECT top $offset a.no_bukti FROM trhtransout_blud a  
        WHERE  a.panjar = '0' AND $init_skpd $where order by CAST (a.no_bukti as NUMERIC)) and 
        a.no_bukti not in (select no_bukti from trhtransout_cmsbank a WHERE  a.panjar = '0' AND $init_skpd $where)
         order by CAST (a.no_bukti as NUMERIC),kd_skpd ";
    ...
    function load_dtransout_transfer_bnk(){ 
        ...
        $sql = "SELECT b.no_bukti,b.tgl_bukti,b.rekening_awal,b.nm_rekening_tujuan,b.rekening_tujuan,
                b.bank_tujuan,b.kd_skpd,b.nilai,(select sum(nilai) from trdtransout_blud_transfer where no_bukti=b.no_bukti and kd_skpd=b.kd_skpd) as total
				FROM trhtransout_blud a INNER JOIN trdtransout_blud_transfer b ON a.no_bukti=b.no_bukti
				AND a.kd_skpd=b.kd_skpd 
       ...
       $no_bukti    = $this->input->post('no_simpan');
		...
		$sql = "select total=isnull(spp,0)+isnull(transaksi,0)+isnull(penagihan,0) from trskpd a left join
									...
									(
										...
										from trhtransout_blud e join trdtransout_blud f on e.no_bukti=f.no_bukti and e.kd_skpd=f.kd_skpd
										where f.kd_kegiatan='$kegiatan' and e.no_bukti<>'$no_bukti' and e.jns_spp ='1' group by f.kd_kegiatan
    ...
    function load_rek_bnk() {                      
       ...
       $stsubah =$this->rka_model->get_nama($kode,'status_ubah','trhrka','kd_skpd');
        ...
        if ($jenis=='1'){
            ...
            $sql = "SELECT b.kd_rek5,b.nm_rek5,
                    (SELECT SUM(c.nilai) FROM trdtransout_blud c LEFT JOIN trhtransout_blud d ON c.no_bukti=d.no_bukti AND c.kd_skpd=d.kd_skpd 
					WHERE c.kd_kegiatan = b.kd_kegiatan AND 
                    d.kd_skpd=a.kd_skpd 
    ...
    function simpan_transout_bnk(){
        ...
        if ($tabel == 'trhtransout_blud') {
            ...
            // Simpan Detail //                                       
                ...
                $sql = "delete from trdtransout_blud_transfer where no_bukti='$nomor' AND kd_skpd='$skpd'";
                $asg = $this->db->query($sql);
				...
				if (!($asg)){
                    ...
                    $sql = "insert into trdtransout_blud_transfer(no_bukti,tgl_bukti,rekening_awal,nm_rekening_tujuan,rekening_tujuan,bank_tujuan,kd_skpd,nilai)"; 
                    $asg = $this->db->query($sql.$csqlrek);                                       
    ...
    function hapus_transout_bnk(){
		...
		if ($asg){
            ...
            $sql = "delete from trdtransout_blud_transfer where no_bukti='$nomor' AND kd_skpd='$kd_skpd'";
            $asg = $this->db->query($sql);
    ...
    function load_trans_trmpot_bnk(){
	   $kode    = $this->session->userdata('kdskpd');
            ...
            FROM trhtransout_blud a
            JOIN trdtransout_blud b ON a.no_bukti = b.no_bukti and a.kd_skpd = b.kd_skpd
            WHERE a.kd_skpd = '$kode' and a.pay in ('BANK','TUNAI') and a.jns_spp in ('1','2','3') 
    ...
    function load_list_upload_perbidang(){
        ...
        WHERE left(a.kd_skpd,7)=left('$skpd',7) $and order by cast(a.no_bukti as int))
	...
	if($tabel == 'trdupload_cmsbank_bidang') {
            // Simpan Detail //                       
                $sql = "delete from trhupload_cmsbank_bidang where no_upload='$nomor' AND kd_skpd='$skpd'";
                $asg = $this->db->query($sql);
				...
				if (!($asg)){
                    ...
                    $skpd = $this->session->userdata('kdskpd'); 
                    $sql = "insert into trhupload_cmsbank_bidang(no_upload,tgl_upload,kd_skpd,total,no_upload_tgl) values ('$nomor','$update','$skpd','$total','$urut_tgl')";
                    $asg = $this->db->query($sql);
                    ...
                    $sql = "UPDATE
                        ...
                        where left(tr_setorpelimpahan_bank_cms.kd_skpd,7)=left('$skpd',7)
                        ";
                    $asg = $this->db->query($sql);
    ...
    function load_hdraf_upload_bidang(){
        ...
        $query1 = $this->db->query("SELECT top $rows a.* FROM trhupload_cmsbank_bidang a               
        where left(a.kd_skpd,7)=left('$skpd',7) $and 
        and a.no_upload not in (SELECT top $offset a.no_upload FROM trhupload_cmsbank_bidang a  
        WHERE left(a.kd_skpd,7)=left('$skpd',7) $and)
        order by cast(a.no_upload as int),a.kd_skpd");		
    ...
    function load_list_validasi_perbidang(){
        ...
        $sql = "SELECT count(*) as total from tr_setorpelimpahan_bank_cms a 
        where left(a.kd_skpd,7)=left('$skpd',7) and status_upload='1' $and " ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
        ...
        where left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='1' $and 
        and a.no_bukti not in (SELECT top $offset a.no_bukti FROM tr_setorpelimpahan_bank_cms a  
        WHERE left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='1' $and order by cast(a.no_bukti as int))
        order by cast(a.no_bukti as int),a.kd_skpd");		
        $result = array();
        $ii     = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function load_list_telahvalidasi_perbidang(){
        ...
        left join trdupload_cmsbank_bidang c on a.no_bukti = c.no_bukti and a.kd_skpd = c.kd_skpd
        left join trvalidasi_cmsbank d on d.no_voucher = c.no_voucher and d.kd_bp = c.kd_bp
        where left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='1' and a.status_validasi='1' $and 
        and a.no_voucher not in (SELECT top $offset a.no_voucher FROM trhtransout_cmsbank a  
        WHERE left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='1' and a.status_validasi='1' $and order by cast(a.no_voucher as int))
        order by cast(d.no_bukti as int),a.tgl_validasi,a.kd_skpd");		
        $result = array();
        $ii     = 0;
        foreach($query1->result_array() as $resulte)
	...
	if($tabel == 'trvalidasi_cmsbank_bidang') {
                    ...
                    if (!($asg)){
                       ...
                       $sql = "UPDATE
                        ...
                        where left(tr_setorpelimpahan_bank_cms.kd_skpd,7)=left('$skpd_ss',7)
                        ";
                        $asg = $this->db->query($sql);
                        if (!($asg)){
                            $msg = array('pesan'=>'0');
  ...
  function jawabpanjar()
   ...
   function tambahpanjar()
    ...
    function load_panjar() {
		...
		$sql = "SELECT count(*) as total from tr_panjar_cmsbank  where kd_skpd='$kd_skpd' $where " ;
        //$sql = "SELECT count(*) as total from trhtransout_blud a where a.kd_skpd='$kd_skpd' and a.jns_spp in ('1','2','3') $where " ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
       	$result["total"] = $total->total; 
		...
		//$sql = "SELECT  * from tr_panjar where kd_skpd='$kd_skpd'";
        $sql = "SELECT top $rows * from tr_panjar_cmsbank where kd_skpd='$kd_skpd' $where and no_panjar not in (SELECT top $offset no_panjar FROM tr_panjar_cmsbank  where kd_skpd='$kd_skpd' $where order by no_panjar)  order by no_panjar";
        $query1 = $this->db->query($sql); 
        $ii = 0;
        foreach($query1->result_array() as $resulte)
        { 
    ...
    function load_listpanjar_upload_cms(){
        ...
        WHERE left(a.kd_skpd,7)=left('$skpd',7) $and order by cast(a.no_kas as int))
            ...
            // Simpan Detail //                       
				...
				if (!($asg)){
                    ...
                    $sql = "UPDATE
                        ...
                        where left(tr_panjar_cmsbank.kd_skpd,7)=left('$skpd',7)
                        ";
                    $asg = $this->db->query($sql);
    ...
    function csv_cmsbank_panjar($nomor=''){
        ...
        where left(a.kd_skpd,7)=left('$skpd',7) and a.no_upload='$nomor' order by c.no_bukti");
    ...
    function load_list_validasi_panjar(){
        ...
        where left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='1' $and 
        and a.no_kas not in (SELECT top $offset a.no_kas FROM tr_panjar_cmsbank a  
        WHERE left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='1' $and order by cast(a.no_kas as int))
        order by cast(a.no_kas as int),a.kd_skpd");
        $result = array();
        $ii     = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function load_listbelum_validasi_panjar(){
        ...
        $sql = "SELECT count(*) as total from tr_panjar_cmsbank a 
        where left(a.kd_skpd,7)=left('$skpd',7) and a.status_upload='1' and a.status_validasi='0' $and " ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
	...
	if($tabel == 'trvalidasi_cmsbank_panjar') {
                    ...
                    if (!($asg)){
                       ...
                       $sql = "UPDATE
                        ...
                        where left(tr_panjar_cmsbank.kd_skpd,7)=left('$skpd_ss',7)
                        ";
                        $asg = $this->db->query($sql);
                        if (!($asg)){
                            $msg = array('pesan'=>'0');
                            ...
                            $sql = "INSERT INTO tr_panjar (no_kas,tgl_kas,no_panjar,tgl_panjar,kd_skpd,pengguna,nilai,keterangan,pay,rek_bank,kd_kegiatan,status,jns,no_panjar_lalu)
                                    SELECT a.no_kas,a.tgl_kas,a.no_panjar,a.tgl_panjar,a.kd_skpd,a.pengguna,a.nilai,a.keterangan,a.pay,a.rek_bank,a.kd_kegiatan,a.status,a.jns,a.no_panjar_lalu
    ...
    function cetak_list_panjarcms(){
        ...
        $thn     = $this->session->userdata('pcThang');
        $tgl     = $this->uri->segment(3);
        $sqlsc="SELECT tgl_rka,provinsi,kab_kota,daerah,thn_ang FROM sclient WHERE kd_skpd=left('$kd_skpd',7)+'.00'";
                 $sqlsclient=$this->db->query($sqlsc);
                 foreach ($sqlsclient->result() as $rowsc)
                {
                    $kab     = $rowsc->kab_kota;
         ...
         $cRet .="<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"1\" cellpadding=\"1\">
           ...
           */
           ...
           $sql = "select z.* from (
            ...
            UNION
            select '2' urut,a.kd_skpd,a.tgl_kas,a.no_kas,a.kd_kegiatan kegiatan,'' rekening, '' ket, 0 terima, a.nilai keluar, '' jns_spp, '' status_upload
            from tr_panjar_cmsbank a where year(a.tgl_kas) = '$thn' and a.tgl_kas='$tgl' and $init_skpd
            UNION
