...
class Welcome extends CI_Controller {
	...
	function index(){
		$this->template->set('title', 'SIMAK-BLUD');
	...
	public function login(){
		$data['logo']		= 'image/soedarso1.png';
		$data['css_data']		= 'css/jr-login.css';
		$data['goverment_name']	= 'PEMPROV KALIMANTAN BARAT';
		$data['app_name']		= 'SIMAK-BLUD';
		$data['dec_app_name']	= 'SISTEM INFORMASI MANAJEMEN AKUNTANSI KEUANGAN<br>BADAN LAYANAN UMUM DAERAH';
		$data['footer']			= '&#9742; Kontak konsultasi : 	<br>Copyright &copy; 2012 MSM Consultant';
		$this->template->set('title', 'FORM LOGIN - SIMAK-BLUD');
		if($_POST){
			...
			if($user == TRUE && ($jr == 0 || $jr == 1)){
				$this->utility->set_log_activity($iduser,$session_id,$skpd,'Succes Login');
				$query = $this->db->query("UPDATE user_blud SET stat_login = '1',last_login = '$last_login' WHERE user_name = '$bar' AND type = '1'");
				redirect('welcome');
