...
class M_sts extends CI_Model {
	...
	function list_no_terima($kd_skpd,$lccr){
		$sql    = "select a.no_terima,a.tgl_terima,a.kd_rek5,a.kd_rek_blud,b.nm_rek5 as nm_rek_blud,a.kd_skpd,a.nilai,a.keterangan,a.kd_kegiatan from tr_terima_blud a left join ms_rek5_blud b on rtrim(a.kd_rek_blud)=rtrim(b.kd_rek5)where kd_skpd='$kd_skpd' AND no_terima NOT IN(select ISNULL(no_terima,'') no_terima from trdkasin_blud where kd_skpd='$kd_skpd') AND no_terima LIKE '%$lccr%' ORDER BY tgl_terima,kd_rek5";
            ...
            if ($asg){
                ...
                if ($asg){
                    ...
                    if(!($asg)){
                        ...
                        $sql = "insert into trdkasin_blud(kd_skpd,no_sts,kd_rek5,rupiah,kd_kegiatan,no_terima,kd_rek_blud,nm_rek_blud) values $lcnilaidet";
                        $asg = $this->db->query($sql); 
						$msg = '2';
						return $msg;
                    }                
	...
	function load_dsts($lcskpd,$kriteria){
		$sql = "SELECT no_sts,kd_skpd,kd_rek5,kd_rek_blud,nm_rek_blud,kd_kegiatan,rupiah,no_terima
		...
		$asg = $this->db->query($sql);
			...
			if ($asg){
				...
				if(!($asg)){
					...
					$sql = "insert into trdkasin_blud(kd_skpd,no_sts,kd_rek5,rupiah,kd_kegiatan,kd_rek_blud,nm_rek_blud) values $lcnilaidet";
					$asg = $this->db->query($sql); 
				}                
			}
		$msg = '2';
